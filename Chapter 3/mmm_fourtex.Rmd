...
To set working directory, please run only part of the following chunk depending on the operating system of your device. To avoid running the codes that are not applicable and receiving error messages, please add $#$ before each line of non-applicable codes in the chunk below. For example, if you are using a Mac, you need to put $#$ before the code *setwd("~/Desktop/fourtex")*. See below. 

```{r warning=FALSE}
# Please run the following codes depending on the operating system on your laptop/PC. 

# If you are using iOS, and your 'FourTex' folder is created on your 'Desktop' on your Mac, you will need to set your working directory to the *FourTex* folder in the following way :

setwd("~/Downloads/fourtex")

# if you are using Windows, and your 'FourTex' folder is created in your H drive: you need to set your working directory to the *FourTex* folder in the following way:

#setwd("H:/downloads/fourtex")
```

Now, we are ready load the data set to R, and perform some exploratory data analysis to get a feel for the data. 
...
```{r}
data<- read.csv(file = "data_fourtex.csv",header = TRUE)
str(data)
```
...
![](diminishing_return.png){width=40%}
...
We can show these media contributions graphically as well. We will use bar plots for this. To obtain the bar plots, we need to install the package called *ggplot2*. If you have installed the *ggplot2* to your computer previously, you can add a hashtag symbol,$#$ and then run the code.See below. 
...
**Discussion**: What is your conclusion with regard to traffic generated and cost incurred? Which media returns the most and least?
